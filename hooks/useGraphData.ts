      ...
      for (const row of rows) {
        ...
        if (!companyMap.has(companyKey)) {
          companyMap.set(companyKey, {
            ...
            total_buy_transactions: row.company_total_buy_transactions ?? 0,
            last_insider_buy_date: row.last_insider_buy_date ?? null,
            avg_return_pct: row.company_avg_return_pct ?? null,
