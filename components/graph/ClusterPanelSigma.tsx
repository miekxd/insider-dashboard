...
interface ClusterPanelSigmaProps {
  graph: Graph | null;
...
export const ClusterPanelSigma = memo(function ClusterPanelSigma({
  graph,
  onHighlight,
  highlighted,
}: ClusterPanelSigmaProps) {
  const clusters = useMemo<ClusterEntry[]>(() => {
    if (!graph) return [];
    const entries: ClusterEntry[] = [];
    graph.forEachNode((key, attrs: any) => {
      if (attrs.nodeType === 'company') {
        const a = attrs as CompanyNodeAttributes;
        if (a.unique_insiders_bought >= 2) {
          entries.push({
            ticker: a.ticker,
            companyName: a.company_name,
            insiderCount: a.unique_insiders_bought,
            nodeKey: key,
          });
        }
      }
  ...
  return (
    <div className="flex flex-col border-t" style={{ borderColor: 'var(--border-primary)', backgroundColor: 'var(--bg-secondary)' }}>
      <div className="flex items-center gap-1.5 px-4 py-2.5 border-b" style={{ borderColor: 'var(--border-primary)' }}>
        ...
        {clusters.map((c) => {
          ...
          return (
            <button
              ...
              onMouseEnter={(e) => { if (!isActive) (e.currentTarget as HTMLElement).style.backgroundColor = 'var(--bg-tertiary)'; }}
              onMouseLeave={(e) => { if (!isActive) (e.currentTarget as HTMLElement).style.backgroundColor = 'transparent'; }}
            >
              <div className="flex flex-col gap-0.5 min-w-0">
                <span className="text-xs font-mono font-semibold" style={{ color: 'var(--text-primary)' }}>{c.ticker}</span>
                <span className="text-xs truncate" style={{ color: 'var(--text-tertiary)', maxWidth: 140 }}>{c.companyName}</span>
              </div>
              <span
                className="text-xs px-1.5 py-0.5 rounded shrink-0 ml-2"
                style={{ backgroundColor: '#E8A02020', color: '#E8A020', fontWeight: 600, border: '1px solid #E8A02040' }}
              >
                {c.insiderCount}×
              </span>
            </button>
